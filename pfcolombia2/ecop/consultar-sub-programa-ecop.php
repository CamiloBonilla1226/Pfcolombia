...
if(isset($_REQUEST["excelX"])){
 ...
 /*
    ...
    */
    ...
    $sql = "SELECT count(*) as conteo FROM sat_reportes WHERE sat_reportes.rep_tip = 347 ".$sqlFiltro;
    $PSN1->query($sql);
    $total_registros = 0;
    if($PSN1->num_rows() > 0){
        if($PSN1->next_record()){
    ...
    $sql_ids = "SELECT sat_reportes.id FROM sat_reportes WHERE sat_reportes.rep_tip = 347 ".$sqlFiltro." ORDER BY sat_reportes.fechaReporte DESC, sat_reportes.id DESC LIMIT ".$inicio.", ".$registros;
    ...
    <form name="form" id="form" method="get" class="form-horizontal">
        <input type="hidden" name="doc" value="consultar-sub-programa-ecop" />
        <div>
            <h3 class="alert alert-info text-center">CONSULTAR REPORTES  (ECOP)</h3>
        </div>
        <div class="cont-tit">
            <div class="hr"><hr></div>
            <div class="tit-cen">
                ...
                <tbody>
                    ...
                    if($total_registros > 0)
                    {
                        ...
                        while($PSN1->next_record())
                        {
                            ...
                            ?><tr class='clickable-row' data-href='index.php?doc=gestionar-sub-programa-ecop&id=<?=$id; ?>' >
                                <!--<td><a href="index.php?doc=gestionar-sub-programa-ecop&id=<?=$id; ?>"><?=str_pad($id, 6, "0", STR_PAD_LEFT); ?></a></td>//-->
                                <td><?=$regional; ?></td>
                                <td><?= $prision; ?></td>
                                <td><?=$nombreUsuario; ?></td>
