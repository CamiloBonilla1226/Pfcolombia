...
if(isset($_REQUEST["excelX"])){
 ...
 /*
    ...
    */
    ...
    $sql = "SELECT count(DISTINCT sat_reportes.id) as conteo FROM sat_reportes WHERE sat_reportes.rep_tip = 347 ".$sqlFiltro;
    $PSN1->query($sql);
    $total_registros = 0;
    if($PSN1->num_rows() > 0){
        if($PSN1->next_record()){
    ...
    $sql_ids = "SELECT sat_reportes.id FROM sat_reportes WHERE sat_reportes.rep_tip = 347 ".$sqlFiltro." ORDER BY sat_reportes.id DESC LIMIT ".$inicio.", ".$registros;
