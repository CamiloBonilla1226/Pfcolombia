...
LEFT JOIN categorias AS CA ON CA.id = C.idSec ";
    ...
    if(isset($_REQUEST["idUsuario"]) && soloNumeros($_REQUEST["idUsuario"]) != ""){
        ...
        $sqlFiltro .= " AND (sat_reportes.idUsuario = '".$buscar_idUsuario."' OR FIND_IN_SET('".$buscar_idUsuario."', sat_reportes.idUsuario))";
    }
    ...
    //    
    $sql .= $sqlFiltro." AND sat_reportes.rep_tip IN (308, 317, 327) ORDER BY sat_reportes.id DESC";
    //
    $PSN1->query($sql);
    //echo $sql;
    if($PSN1->num_rows() > 0){
